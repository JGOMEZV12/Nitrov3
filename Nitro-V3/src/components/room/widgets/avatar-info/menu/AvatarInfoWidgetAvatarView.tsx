...
import { AvatarInfoUser, DispatchUiEvent, ChatMessageTypeEnum, GetOwnRoomObject, GetUserProfile, LocalizeText, MessengerFriend, ReportType, RoomWidgetUpdateChatInputContentEvent, SendMessageComposer } from '../../../../../api';
import { Flex } from '../../../../../common';
import { useChatInputWidget, useFriends, useHelp, useRoom, useSessionInfo, useWiredTools } from '../../../../../hooks';
        ...
        if(name)
        {
            switch(name)
            {
                ...
                case 'unignore':
                    ...
                    break;
                 case 'kiss':
                    sendChat(`:besar ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;
                case 'hugging':
                    sendChat(`:abrazar ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;   
                case 'spanking':
                    sendChat(`:nalgada ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;  
                case 'violate':
                    sendChat(`:violar ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;   
                case 'spit':
                    sendChat(`:escupir ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;
                case 'robbery':
                    sendChat(`:robar ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;   
                case 'hit':
                    sendChat(`:golpe ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;    
                case 'slap':
                    sendChat(`:cachetada ` + avatarInfo.name, ChatMessageTypeEnum.CHAT_DEFAULT, "", 1);
                    break;         
                case 'kick':
        ...
        <ContextMenuView category={ RoomObjectCategory.UNIT } collapsable={ true } objectId={ avatarInfo.roomIndex } userType={ avatarInfo.userType } onClose={ onClose }>
            ...
            { (mode === MODE_NORMAL) &&
                <>
                    ...
                    { !canRequestFriend(avatarInfo.webID) &&
                     ...
                     <ContextMenuListItemView onClick={ event => processAction('actions') }>
                            { LocalizeText('infostand.link.actions') }
                            <FaChevronRight className="right fa-icon" />
                        </ContextMenuListItemView>       
            ...
            { (mode === MODE_RELATIONSHIP) &&
                <>
                 ...
                 { (mode === MODE_ACTIONS) &&
                <>
                    <ContextMenuListItemView onClick={ event => processAction('kiss') }>
                        { LocalizeText('infostand.button.actions.kiss') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('hugging') }>
                         { LocalizeText('infostand.button.actions.hugging') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('violate') }>
                         { LocalizeText('infostand.button.actions.violate') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('spanking') }>
                         { LocalizeText('infostand.button.actions.spanking') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('robbery') }>
                         { LocalizeText('infostand.button.actions.robbery') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('spit') }>
                         { LocalizeText('infostand.button.actions.spit') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('hit') }>
                         { LocalizeText('infostand.button.actions.hit') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('slap') }>
                         { LocalizeText('infostand.button.actions.slap') }
                    </ContextMenuListItemView>
                    <ContextMenuListItemView onClick={ event => processAction('back') }>
                        <FaChevronLeft className="left fa-icon" />
                        { LocalizeText('generic.back') }
